...
command_line::add_arg(desc_cmd_sett, arg_enable_cors);
    ...
    logger(INFO, BRIGHT_BLUE) <<
        ...
        "\n                                                                                   \n"
        " ÛÛÛÛÛ   ÛÛÛÛÛ                                 ÛÛÛÛÛÛÛÛÛ            ÛÛÛ            \n"
        "°°ÛÛÛ   °°ÛÛÛ                                 ÛÛÛ°°°°°ÛÛÛ          °°°             \n"
        " °ÛÛÛ    °ÛÛÛ  ÛÛÛÛÛ ÛÛÛÛ ÛÛÛÛÛÛÛÛ   ÛÛÛÛÛÛ  ÛÛÛ     °°°   ÛÛÛÛÛÛ  ÛÛÛÛ  ÛÛÛÛÛÛÛÛ  \n"
        " °ÛÛÛÛÛÛÛÛÛÛÛ °°ÛÛÛ °ÛÛÛ °°ÛÛÛ°°ÛÛÛ ÛÛÛ°°ÛÛÛ°ÛÛÛ          ÛÛÛ°°ÛÛÛ°°ÛÛÛ °°ÛÛÛ°°ÛÛÛ \n"
        " °ÛÛÛ°°°°°ÛÛÛ  °ÛÛÛ °ÛÛÛ  °ÛÛÛ °ÛÛÛ°ÛÛÛÛÛÛÛ °ÛÛÛ         °ÛÛÛ °ÛÛÛ °ÛÛÛ  °ÛÛÛ °ÛÛÛ \n"
        " °ÛÛÛ    °ÛÛÛ  °ÛÛÛ °ÛÛÛ  °ÛÛÛ °ÛÛÛ°ÛÛÛ°°°  °°ÛÛÛ     ÛÛÛ°ÛÛÛ °ÛÛÛ °ÛÛÛ  °ÛÛÛ °ÛÛÛ \n"
        " ÛÛÛÛÛ   ÛÛÛÛÛ °°ÛÛÛÛÛÛÛ  °ÛÛÛÛÛÛÛ °°ÛÛÛÛÛÛ  °°ÛÛÛÛÛÛÛÛÛ °°ÛÛÛÛÛÛ  ÛÛÛÛÛ ÛÛÛÛ ÛÛÛÛÛ\n"
        "°°°°°   °°°°°   °°°°°ÛÛÛ  °ÛÛÛ°°°   °°°°°°    °°°°°°°°°   °°°°°°  °°°°° °°°° °°°°° \n"
        "                ÛÛÛ °ÛÛÛ  °ÛÛÛ                                                     \n"
        "               °°ÛÛÛÛÛÛ   ÛÛÛÛÛ                                                    \n"
        "                °°°°°°   °°°°°                                                     \n" << ENDL;

    logger(INFO, BRIGHT_BLUE) << "Welcome! " << CryptoNote::CRYPTONOTE_NAME << " v" << PROJECT_VERSION_LONG;

    if (command_line_preprocessor(vm, logger)) {
      return 0;
    }

    logger(INFO, BRIGHT_BLUE) << "Module folder: " << argv[0];

    bool testnet_mode = command_line::get_arg(vm, arg_testnet_on);
    if (testnet_mode) {
      logger(INFO, BRIGHT_BLUE) << "Starting in testnet mode!";
    }
...
for (const auto& cp : checkpoint_input) {
    ...
    logger(INFO, BRIGHT_BLUE) << "Initializing core...";
    ...
    ccore.load();
    logger(INFO, BRIGHT_BLUE) << "Core initialized OK";
    ...
    logger(INFO, BRIGHT_BLUE) << "Initializing p2p server...";
    if (!p2psrv.init(netNodeConfig)) {
      logger(ERROR, BRIGHT_RED) << "Failed to initialize p2p server.";
      return 1;
    }

    logger(INFO, BRIGHT_BLUE) << "P2p server initialized OK";

    if (!command_line::has_arg(vm, arg_console)) {
      dch.start_handling();
    }

    logger(INFO, BRIGHT_BLUE) << "Starting core rpc server on address " << rpcConfig.getBindAddress();
    rpcServer.start(rpcConfig.bindIp, rpcConfig.bindPort);
  rpcServer.setFeeAddress(command_line::get_arg(vm, arg_set_fee_address));
rpcServer.enableCors(command_line::get_arg(vm, arg_enable_cors));
    logger(INFO, BRIGHT_BLUE) << "Core rpc server started ok";
    ...
    logger(INFO, BRIGHT_BLUE) << "Starting p2p net loop...";
    p2psrv.run();
    logger(INFO, BRIGHT_BLUE) << "p2p net loop stopped";

    dch.stop_handling();

    //stop components
    logger(INFO, BRIGHT_BLUE) << "Stopping core rpc server...";
    rpcServer.stop();

    //deinitialize components
    logger(INFO, BRIGHT_BLUE) << "Deinitializing p2p...";
    p2psrv.deinit();
  ...
  logger(INFO, BRIGHT_BLUE) << "Node stopped.";
  return 0;
}
